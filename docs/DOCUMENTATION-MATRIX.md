...
| `docs/ENTENDENDO-O-WIRS.md` | Didático | Como detecções entregues funcionam | Informativa baseada em código | Atual | Corrigido | Não | Não | Código + Issues fechadas | Analistas | Detecção entregue ou removida | Referência a sessão/arquivo inexistente | Referência tornada reproduzível; schema 2.0 e YARA no `scan` explicados | 2026-09-15 |
