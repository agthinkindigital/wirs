# Documentação do WIRS

- [`PRODUCT-CHARTER.md`](PRODUCT-CHARTER.md): identidade, limites e ordem de expansão.
- [`DOCUMENTATION-MATRIX.md`](DOCUMENTATION-MATRIX.md): autoridade e sincronização por assunto.
- [`ARCHITECTURE.md`](ARCHITECTURE.md): como o sistema foi pensado e como as peças se encaixam.
- [`ENTENDENDO-O-WIRS.md`](ENTENDENDO-O-WIRS.md): o porquê de cada peça, explicado slice por slice.
- [`WIRS_MASTER_SPEC_PT-BR.md`](../WIRS_MASTER_SPEC_PT-BR.md): especificação viva (autoridade de produto e arquitetura).
- [`CONTEXT.md`](../CONTEXT.md): glossário do domínio.
- [`adr/`](adr/): decisões arquiteturais.
- [`providers/`](providers/): contrato de cada provider externo.
- [`future/`](future/): visões futuras não normativas.
- [`case-studies/`](case-studies/): lições de incidentes, sem redefinir o produto.
- [`audits/`](audits/): auditorias de produto, arquitetura e governança.
- [`PROMPT-REALINHAMENTO-WIRS.md`](PROMPT-REALINHAMENTO-WIRS.md): prompt normativo da auditoria.
- [`WIRS-MATRIZ-DOCUMENTAL-TEMPLATE.md`](WIRS-MATRIZ-DOCUMENTAL-TEMPLATE.md): template da matriz.
- [`agents/README.md`](agents/README.md): operação com agentes (domínio, tracker, triage).

## Autoridade por assunto

- Produto: [`PRODUCT-CHARTER.md`](PRODUCT-CHARTER.md).
- Vocabulário: [`../CONTEXT.md`](../CONTEXT.md).
- Requisitos: [`../WIRS_MASTER_SPEC_PT-BR.md`](../WIRS_MASTER_SPEC_PT-BR.md).
- Implementação real: código + testes reproduzíveis.
- Ordem: [`../ORCHESTRATOR-ROADMAP.md`](../ORCHESTRATOR-ROADMAP.md).
- Estado: [`../ESTADO_ORQUESTRATOR.md`](../ESTADO_ORQUESTRATOR.md).

Quando houver divergência, registre-a e resolva-a na fonte competente.
